...
####in order to compare dimenson 1 with 2, create a np array for use with scipi.stats.linregresion. Need to reshape the numpy array to format [2,N]###
data_1_2 = np.array([list_data[0],list_data[1]])
data_1_2_reshape = np.reshape(data_1_2, (113965,2))
#### linear regresion module in scipy takes as input a singel np array and output a tupple with slope, y-intercept r and more, all collected in res###
res = stats.linregress(data_1_2_reshape)
plt.scatter(list_data[0], list_data[1], s=0.1)
####intersting way to plot a line given slope and y-intercept: multiply X-range by solopw and add y-intercept
